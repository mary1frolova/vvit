...
class InputDialog(QDialog):
    def __init__(self, title, fields, parent=None):
        super().__init__(parent) #обращаемся к классу род.
        self.setWindowTitle(title) #конкретно title

        self.fields = fields # создаём поля
        self.line_edits = {} #vvod v liniu

        layout = QFormLayout() # sozdaet yacheiki
...
class DatabaseApp(QWidget):
    def __init__(self):
        ...
        self.connection = psycopg2.connect(database="vvit_8",
                                           user="postgres",
                                           password="next",
                                           host="localhost")
        self.initUI()
